...
- **`whimsical-inventing-wozniak` cannot be trusted to run PHP suites** — autoload resolves to `/home/kbarendrecht/development/scienta/vendor/composer/ClassLoader.php`, outside the worktree, so a suite run there loads main's code. §7's post-WIP table assumes otherwise.
