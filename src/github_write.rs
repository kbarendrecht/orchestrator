...
mod tests {
    ...
    fn the_footer_goes_on_its_own_paragraph() {
        assert_eq!(
            with_footer("Opgelost in de mapper."),
            "Opgelost in de mapper.\n\n(via orchestrator)"
        );
    }
    ...
    fn a_reply_path_threads_under_the_comment_it_answers() {
        ...
        assert_eq!(
            reply_path("ScientaNL", "Scienta", 34838, 3751162734),
            "repos/ScientaNL/Scienta/pulls/34838/comments/3751162734/replies"
        );
    }
    ...
    fn a_reaction_path_is_not_nested_under_the_pr() {
        assert_eq!(
            reaction_path("ScientaNL", "Scienta", 3751162734),
            "repos/ScientaNL/Scienta/pulls/comments/3751162734/reactions"
        );
    }

    #[test]
    fn only_reviewers_with_nothing_open_are_re_requested() {
        // Melvin's five handled while Luuk's two are open re-requests Melvin
        // alone.
        assert_eq!(
            ready_to_rerequest(&["melvin", "luuk"], &["luuk"]),
            vec!["melvin"]
        );
        assert!(ready_to_rerequest(&["luuk"], &["luuk"]).is_empty());
        assert_eq!(
            ready_to_rerequest(&["melvin", "melvin"], &[]),
            vec!["melvin"]
        );
    }
}
