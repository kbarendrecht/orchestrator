...
pub struct CapabilityConfig {
    ...
    /// play. Point it at an application class (`Scienta\\...`) for the stronger
...
mod tests {
    ...
    fn maps_a_worktree_path_into_the_container() {
        ...
        let main = Path::new("/home/k/development/scienta");
        let wt = Path::new("/home/k/development/scienta/.claude/worktrees/invoice");
        assert_eq!(
            container_path(&cfg, wt, main).as_deref(),
            Some("/scienta/.claude/worktrees/invoice")
        );
        assert_eq!(container_path(&cfg, main, main).as_deref(), Some("/scienta"));
    }
    ...
    fn a_containerised_suite_reports_the_mapped_docker_exec_form() {
        ...
        assert!(cmd.contains(&"/scienta/.claude/worktrees/x".to_string()), "{cmd:?}");
